let TokenContract = artifacts.require('BNS');

const truffleAssert = require('truffle-assertions');

contract("Burnable Tokens", async (accounts) => {
  const TotalSupply = '250000000000000000';
  ...
  describe('transfer', () => {
    ...
    it('spender transfers from the custody account to user1', async () => {
      ...
      expect(balance.toString()).to.equal('249999999999998500');
    });
  });
  ...
  describe('burn baby burn!', () => {
    it('current owner can burn tokens', async () => {
      ...
      expect(balance.toString()).to.equal('249999999999997500');
      balance = await myContract.totalSupply();
      expect(balance.toString()).to.equal('249999999999999000');
    });
    ...
    it('spender can also burn its own tokens', async () => {
      ...
      balance = await myContract.totalSupply();
      expect(balance.toString()).to.equal('249999999999998900');
    });
    ...
    it('spender can also burn tokens from its allowance', async () => {
      ...
      balance = await myContract.balanceOf(currentOwner);
      expect(balance.toString()).to.equal('249999999999997000');
      balance = await myContract.totalSupply();
      expect(balance.toString()).to.equal('249999999999998400');
    });
  });
});